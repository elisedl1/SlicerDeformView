...
![Overview of DeformView 3D Slicer Module. Left: user interface for the proposed module Right: dense colour map (red is higher deformation) with cursor displaying point-wise deformation magnitude on hover in sagittal (top), axial (middle), and coronal (bottom) views \label{fig:UI_overview}](figures/UI_overview.png)
